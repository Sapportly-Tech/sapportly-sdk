/**
 * Sapportly TypeScript SDK — quick start.
 *
 * Run with: npx tsx examples/quickstart.ts
 * Requires: SAPPORTLY_API_KEY (or legacy SUPPORTLY_API_KEY) with scopes messages:write and conversations:read
 */
import {
  collect,
  SapportlyClient,
...
const apiKey = process.env.SAPPORTLY_API_KEY?.trim() || process.env.SUPPORTLY_API_KEY?.trim();
if (!apiKey) {
  console.error("Set SAPPORTLY_API_KEY (or SUPPORTLY_API_KEY) to run this example.");
  process.exit(1);
}

const client = new SapportlyClient({ apiKey, timeoutMs: 10_000 });
...
try {
  ...
  if (error instanceof SapportlyPermissionError) {
    console.warn("this key has no team:read scope — skipping roles");
  } else if (error instanceof SapportlyRateLimitError) {
